...
#define MAX(x,y)	 (x>y?x:y)
	...
	Mat		features2( const Mat &image, const int sbin, const int pad[2] )
	{
		...
		for( int i=0; i<18; i++ ){
			...
			Rect m = Rect( Rect(i*blocks[1],0,blocks[1],blocks[0]) );
			//histmat[i] = hist_tmp( Rect(i*blocks[1],0,blocks[1],blocks[0]) );
			histmat[i] = hist_tmp( m);
		...
		for (int yy = 1; yy < visible[0]-1; yy++) { // each row
			...
			for (int xx = 1; xx < visible[1]-1; xx++) { // each col
				...
				float	*hist_pt = hist[best_o] + iyp*hist_step[best_o] + ixp;//mark
				...
				if (ixp >= 0 && iyp+1 < blocks[0]) {
					*(hist_pt + hist_step[best_o]) += vx1*vy0*v;
				}

				if (ixp+1 < blocks[1] && iyp+1 < blocks[0]) {
					*(hist_pt + hist_step[best_o] + 1) += vx0*vy0*v;
				}
				/*if( ixp<0 ){
					ixp = 0;
					vx1 = 0;
