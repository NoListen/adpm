...
#include <xmmintrin.h> // using SSE acceleration
...
Mat		features( const Mat &image, const int sbin , const Mat &T1, const Mat &T2)
{
	...
	for (int yy = 1; yy < visible[0]-1; yy++) { // each row
		...
		for (int xx = 1; xx < visible[1]-1; xx++) { // each col
			...
			for( int i=0; i!=3; i++ ){
				dxs = Right[i] - Left[i];
				dys = Down[i] - Up[i];
				vs = T2.at<float>(dxs+255,dys+255);
				if( vs>=v ){
					v = vs;
					dx = dxs;
					dy = dys;
			...
			//float best_dot = 0;
			int best_o = T1.at<int>(dx+255,dy+255);
	...
	Mat	feat_mat = Mat::zeros( out[0], out[1], CV_32FC(out[2]) );
	float * const feat_data = feat_mat.ptr<float>(0,0);
	const int feat_step = feat_mat.step1();

	// SSE acceleration ==== special issue!
	Mat float4mat(1,4,CV_32FC1);
	float * const float4_aligned = float4mat.ptr<float>(0,0); // n1 = *(float4_n+3), n2 = *(float4_n+2), n3 = *(float4_n+1), n4 = *(float4_n)
	__m128 n1234, t1234, h1234, tmp128;
	const __m128 x128 = _mm_set_ps( 0.2f, 0.2f, 0.2f, 0.2f );
	...
	for( int y=0; y<out[0]; y++ ){
		...
		for( int x=0; x<out[1]; x++ ){
			// 			
			*(float4_aligned) = *Normpt;
			*(float4_aligned+2) = *(Normpt+1);
			*(float4_aligned+1) = *(Normpt+norms_step);
			*(float4_aligned+3) = *(Normpt+norms_step+1);
			n1234 = _mm_load_ps( float4_aligned );
			Normpt++;

			// 18 contrast-sensitive features
			t1234 = _mm_setzero_ps();
			for( int i=0; i<18; i++ ){
				float	src = *( hist[i]+(y+1)*hist_step[i]+x+1 );
				tmp128 = _mm_set_ps( src, src, src, src );
				tmp128 = _mm_mul_ps( tmp128, n1234 );
				h1234 = _mm_min_ps( tmp128, x128 );		
				_mm_store_ps( float4_aligned, h1234 );
				*(dst++) = 0.5f * ( float4_aligned[0]+float4_aligned[1]+float4_aligned[2]+float4_aligned[3] );
				t1234 = _mm_add_ps( t1234, h1234 );
			}
			...
			// 4 texture features
			tmp128 = _mm_set_ps( 0.2357f, 0.2357f, 0.2357f, 0.2357f );
			tmp128 = _mm_mul_ps( tmp128, t1234 );
			_mm_store_ps( float4_aligned, tmp128 );
			*(dst++) = *(float4_aligned+3);
			*(dst++) = *(float4_aligned+2);
			*(dst++) = *(float4_aligned+1);
			*(dst++) = *(float4_aligned);
