...
float yuConv( const Mat &A, const Mat &B, const int x, const int y )
{
	...
	if( Asub.channels()==32 && B.channels()==32 )
		...
		if (Asub.channels()==1 && B.channels() == 256)
		{
			for (int i = 0; i < B.rows; ++i)
				for (int j = 0; j < B.cols; ++j)
				{
					int q = Asub.at<int>(i,j);
					const float * t = B.ptr<float>(i,j);
					val += *(t+q);
				}
		}
		else val = (float)( B.dot( Asub ) );
	return	val;
}
			  ...
			  const CSC_MODEL &model, const FEATURE_PYRAMID &pyra ) 
{
	...
	const Mat &A = ( pca==0 ? pyra.feat[L] : pyra.qtfeat[L] );
	const Mat &B = ( pca==0 ? model.partfilters[filterind].w : model.partfilters[filterind].wlut );
